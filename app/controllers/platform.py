...
def PlatformUpdate():
    try:
        ...
        bodyJson = request.json
        err = validators.Platform(bodyJson)
        collPlatform = models.Platforms.objects(id=ObjectId(platformId), isActive=True, isDelete=False)
        if not collPlatform:
            return responses.Make(
                Status=HTTPStatus.BAD_REQUEST.value,
                Message="error",
...
def PlatformDelete():
    try:
        ...
        userId = request.args["userId"]
        collPlatform = models.Platforms.objects(
            id=ObjectId(platformId), isActive=True, isDelete=False, updatedBy=ObjectId(userId)).delete()
        if not collPlatform:
            return responses.Make(
                Status=HTTPStatus.OK.value,
                Message="error",
